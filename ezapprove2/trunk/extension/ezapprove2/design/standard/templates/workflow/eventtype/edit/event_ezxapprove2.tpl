...
<div class="element">
    ...
    <option value="-1" {cond( $eventData.selected_section_list|contains( -1 ), 'selected="selected"', '' )}>{'All sections'|i18n( 'design/admin/workflow/eventtype/edit' )}</option>
...
<fieldset>
<legend><label><input type="radio" name="ApproveType_{$event.id}" value="1" {cond( $eventData.approve_type|eq(1), 'checked="checked"', '' )} />{'Users select approvers themselves'|i18n( 'design/admin/workflow/eventtype/edit' )}</label></legend>
{'Users select who should approve the content when publishing'|i18n('design/admin/workflow/eventtype/edit')}
<div class="block">
    <div class="element">
        {'Required number of users to approve content'|i18n( 'design/admin/workflow/eventtype/edit' )}
    </div>
    <div class="element">
        <select name="RequiredNumberApproves_{$event.id}">
            <option value="-1" {cond( $eventData.num_approve_users|eq(-1), 'selected="selected"', '')}>{'Any'|i18n( 'design/admin/workflow/eventtype/edit' )}</option>
            {for 1 to 10 as $num}
...
<div class="block">
    ...
    <fieldset>
        <legend>{'Allow approvers to be added while approving content.'|i18n( 'design/admin/workflow/eventtype/edit' )}</legend>
...
<fieldset>
<legend><label><input type="radio" name="ApproveType_{$event.id}" value="0" {cond( $eventData.approve_type|eq(0), 'checked="checked"', '' )} />{'Pre selected approvers'|i18n( 'design/admin/workflow/eventtype/edit' )}</label></legend>
